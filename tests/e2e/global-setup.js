...
async function waitForServices() {
  ...
  const services = [
    ...
    {
      name: 'API',
      url: (process.env.E2E_API_URL || 'http://localhost:8000') + '/health',
      timeout: 30000
    }
  ];
  ...
  for (const service of services) {
    try {
      ...
      console.error(`❌ ${service.name} (${service.url}) is not ready:`, error.message);
      throw new Error(`Service ${service.name} at ${service.url} is not available`);
    }
  }
