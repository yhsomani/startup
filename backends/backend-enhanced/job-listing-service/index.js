...
const auth = new AuthMiddleware(new SecurityManager());
...
class JobListingService extends EnhancedServiceWithTracing {
    constructor() {
        ...
        // Create Express app (required for all services)
        this.app = express();
    ...
    setupMiddleware() {
        ...
        this.app.use('/api/v1/jobs', auth.authenticate());
        this.app.use('/api/v1/jobs/:id/apply', auth.authenticate());
    }
    ...
    async start() {
        const startupSpan = this.tracer ? this.tracer.startSpan(`${this.config.serviceName}.startup`) : null;
        try {
            return new Promise((resolve) => {
                this.server = this.app.listen(this.config.port, () => {
                    this.logger.info(`🚀 ${this.config.serviceName} is running on port ${this.config.port}`);
                    if (startupSpan) {
...
// Initialize and start service if run directly
