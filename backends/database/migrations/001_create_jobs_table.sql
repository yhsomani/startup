-- Jobs and employment-related tables
...
CREATE TABLE IF NOT EXISTS jobs (
    ...
    remote_type VARCHAR(50) DEFAULT 'onsite', -- onsite, remote, hybrid
    employment_type VARCHAR(50) DEFAULT 'full-time', -- full-time, part-time, contract, internship
    experience_level VARCHAR(50), -- entry, junior, mid, senior, executive
...
-- Job applications table (basic - extended in applications service)
CREATE TABLE IF NOT EXISTS job_applications (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    job_id UUID REFERENCES jobs(id) ON DELETE CASCADE,
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    status VARCHAR(50) DEFAULT 'pending', -- pending, viewed, interviewing, accepted, rejected, withdrawn
