    ...
    return (
        ...
        >
            ...
            <div className="fixed inset-0 pointer-events-none z-[-1] overflow-hidden">
                <div className="absolute top-[20%] left-[-5%] w-[500px] h-[500px] bg-[var(--color-secondary)]/5 blur-[100px] rounded-full animate-pulse" />
                <div className="absolute bottom-[20%] right-[-5%] w-[500px] h-[500px] bg-[var(--color-primary)]/5 blur-[100px] rounded-full animate-pulse delay-500" />
            </div>

            {/* Matrix Filters Sidebar */}
            <aside className="w-full lg:w-80 space-y-8 shrink-0">
                <motion.div variants={signalVariants} className="glass-panel p-8 rounded-[2.5rem] border-white/10 space-y-8 sticky top-24 bg-black/40 shadow-xl">
                    ...
                    <div className="space-y-4">
                        <div className="relative">
                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-primary)]" size={16} />
                            <input
                                type="text"
                                placeholder="Refine Query..."
                                className="w-full bg-black/40 border-white/5 rounded-xl pl-12 pr-4 py-4 text-[10px] font-black italic uppercase tracking-widest text-white outline-none focus:border-[var(--color-secondary)]/40 transition-all font-bold"
                                value={searchQuery}
                        ...
                        {MATRIX_FILTERS.map(filter => (
                            <button
                                ...
                                className={cn(
                                    ...
                                    activeFilter === filter
                                        ? "bg-[var(--color-secondary)]/10 text-[var(--color-secondary)] border-[var(--color-secondary)]/30 shadow-[0_0_20px_var(--color-secondary-glow)]"
                                        : "bg-white/5 text-slate-500 border-white/5 hover:border-white/10 hover:text-white"
                                )}
                            >
                                {filter}
                                {activeFilter === filter && <div className="size-1.5 bg-[var(--color-secondary)] rounded-full animate-ping" />}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="pt-8 border-t border-white/5 space-y-6">
                        ...
                        <Button variant="ghost" fullWidth size="sm" className="bg-white/5 border-white/5 text-slate-500 hover:text-white text-[9px] font-black tracking-widest italic">
                            RESET MATRIX SCAN
                        </Button>
                    </div>
                </motion.div>
            ...
            <main className="flex-1 space-y-8">
                {/* Discovery Header */}
                <motion.div variants={signalVariants} className="relative p-10 glass-panel rounded-[3rem] border-white/10 overflow-hidden group shadow-2xl bg-black/40">
                    <div className="absolute inset-0 bg-gradient-to-r from-[var(--color-secondary)]/10 via-transparent to-transparent pointer-events-none" />
                    <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                        ...
                        <div className="flex gap-4">
                            <div className="glass-panel px-6 py-4 rounded-2xl border-white/5 flex flex-col items-center justify-center min-w-[100px] bg-black/20">
                                <span className="text-xl font-black text-[var(--color-secondary)] italic leading-none">12</span>
                                <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest mt-1">New Hits</span>
                            </div>
                            <div className="glass-panel px-6 py-4 rounded-2xl border-white/5 flex flex-col items-center justify-center min-w-[100px] bg-black/20">
                                <span className="text-xl font-black text-[var(--color-primary)] italic leading-none">98%</span>
                    ...
                    <AnimatePresence mode="popLayout">
                        {loading ? (
                            Array.from({ length: 5 }).map((_, i) => (
                                <div key={i} className="h-48 glass-panel rounded-[2rem] border-white/5 animate-pulse bg-white/5" />
                            ))
                        ) : jobs.length > 0 ? (
                            jobs.map((job) => (
                                <motion.div
                                    ...
                                    className="glass-panel p-10 rounded-[2.5rem] border-white/5 hover:border-[var(--color-secondary)]/30 transition-all duration-300 group cursor-pointer relative overflow-hidden bg-black/40 shadow-xl"
                                >
                                    {/* Affinity Glow */}
                                    <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-[var(--color-secondary)]/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                                    
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative z-10">
                                        <div className="flex gap-8 items-start">
                                            <div className="size-16 rounded-3xl bg-gradient-to-br from-[var(--color-secondary)] to-[var(--color-primary)] p-[2px] shadow-2xl shrink-0 shadow-[var(--color-secondary-glow)]">
                                            ...
                                            <div className="space-y-4">
                                                <div className="space-y-1">
                                                    <h3 className="text-2xl font-black text-white italic tracking-tighter group-hover:text-[var(--color-secondary)] transition-colors">{job.title}_</h3>
                                                    <div className="flex items-center gap-3">
                                                        <span className="text-[10px] font-black text-[var(--color-primary)] uppercase tracking-widest italic">{job.company}</span>
                                        ...
                                        <div className="flex flex-col items-end gap-6 w-full md:w-auto">
                                            <div className="text-right">
                                                ...
                                                <div className="text-[9px] font-black text-[var(--color-secondary)] uppercase tracking-widest mt-1 italic">Neural Compatibility</div>
                                            </div>
                                            ...
                                            <div className="flex gap-3 w-full md:w-auto">
                                                ...
                                                >
                                                    <Bookmark size={18} className={cn(bookmarked.includes(job.id) ? "fill-[var(--color-primary)] text-[var(--color-primary)]" : "text-slate-500")} />
                                                </Button>
                                                <Button variant="primary" size="sm" className="px-8 text-[10px] italic font-black shadow-[0_0_20px_var(--color-secondary-glow)]">
                                                    ACQUIRE ROLE_
                                                </Button>
                                            </div>
                                        </div>
                            ...
                            <div className="py-40 flex flex-col items-center text-center glass-panel rounded-[3rem] border-white/5 border-dashed bg-black/20">
