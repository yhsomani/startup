...
function createEnhancedProxy(serviceName, serviceUrl, options = {}) {
  ...
  return createProxyMiddleware({
    ...
    onError: (err, req, res) => {
      ...
      return buildErrorResponse('SERVICE_UNAVAILABLE', `${service.name} is currently unavailable`, req);
    },
...
app.get('/health', async (req, res) => {
  ...
  const overallStatus = Object.values(healthResults).every(r => r.success) ? 'healthy' : 'degraded';
  ...
  app.use(route, async (req, res, next) => {
    try {
      ...
      if (service.status === 'unhealthy') {
        return res.status(503).json(buildErrorResponse('SERVICE_UNAVAILABLE', `${service.name} is currently unavailable`, req));
      }
      ...
      return res.status(503).json(buildErrorResponse('SERVICE_UNAVAILABLE', `${service.name} is temporarily unavailable`, req));
    }
  });
});
...
app.use('/api/v1/courses', createFallbackMiddleware('/api/v1/courses', fallbackResponses['/api/v1/courses']));
app.use('/api/v1/challenges', createFallbackMiddleware('/api/v1/challenges', fallbackResponses['/api/v1/challenges']));
app.use('/api/v1/notifications', createFallbackMiddleware('/api/v1/notifications', fallbackResponses['api/v1/notifications']));
...
function metricsMiddleware(req, res, next) {
  ...
  res.on('finish', () => {
    ...
    if (serviceTimes.length > 0) {
      metrics.responseTime.byService[serviceName] = serviceTimes.reduce((a, b) => a + b, 0) / serviceTimes.length;
    }
    metrics.responseTime.average = metrics.responseTime.total / Object.keys(metrics.responseTime.byService).length || 1;
  });
...
app.use((err, req, res, next) => {
  ...
  const isServiceError = err.message?.includes('ECONNREFUSED') || 
...
async function startServer() {
  ...
  const healthResults = await checkAllServices();
  const healthyServices = Object.entries(healthResults).filter(([_, result]) => result.success).length;
