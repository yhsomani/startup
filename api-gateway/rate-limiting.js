            ...
            try {
                ...
                const checks = await Promise.all([
                    ...
                    this.checkLimit(
                        "endpoint",
                        endpointKey,
                        this.options.defaultLimits.endpoint[endpointKey]
                    ),
                ]);
                ...
                res.set({
                    ...
                    "X-RateLimit-Remaining": Math.max(
                        0,
                        this.options.defaultLimits.ip.maxRequests -
                            (await this.getCurrentCount(clientKey))
                    ),
                    "X-RateLimit-Reset": new Date(
                        Date.now() + this.options.defaultLimits.ip.windowMs
                    ).toISOString(),
                });
        ...
        try {
            ...
            if (currentCount >= limitConfig.maxRequests) {
                ...
                const resetTime = oldest[1]
                    ? parseInt(oldest[1], 10) + limitConfig.windowMs
                    : now + limitConfig.windowMs;
    ...
    async blacklist(key) {
        ...
        try {
            await this.redis.setex(
                blacklistKey,
                Math.ceil(this.options.blacklistDuration / 1000),
                "1"
            );
